...
from dataset_brain import Dataset_gan_T
...
def main():
    file_path='./npy_train/train_T18T2.npy'
    train_data=Dataset_gan_T(file_path)
    batch_size=1
    ...
    model_save='./Weights/unet_miso_t.pth'
    for epoch in range(EPOCH):
        ...
        for i,(flair,t18t2,label) in enumerate(train_loader):
            info_c_ =torch.randint(3,(t18t2.size(0),))
            info_c = label2onehot(info_c_,3).cuda()
            img=torch.zeros(t18t2.size(0),t18t2.size(1),t18t2.size(2),t18t2.size(3))
        ...
        log.to_csv('./Weights/logunet_miso_t.csv', index=False)
